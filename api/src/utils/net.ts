...
export default class Net {
  static async addRoute(subnet: string, via: string, i = 'wg0') {
    try {
      const { stdout } = await CLI.exec(`ip route add ${subnet} via ${via} dev ${i}`);
  ...
  static async delRoute(subnet: string, via: string) {
    try {
      const { stdout } = await CLI.exec(`ip route del ${subnet}`);
  ...
  static async isReachable(ip: string): Promise<boolean> {
    return new Promise(async (resolve) => {
      ...
      try {
        ...
        if ( stdout && stdout.indexOf('1 packets transmitted, 1 packets received, 0% packet loss') !== -1) {
          resolve(true)
        } else {
          resolve(false)
        }
  ...
  static async lookupIncludesThisServer(domain: string): Promise<boolean> {
    ...
    return (lookup[0] || lookup[1]) && lookup.flat().includes(config.wireguard.host);
  }

  static async checkPort(host: string, port: number, resolver?: string, timeout = 3000): Promise<boolean> {
