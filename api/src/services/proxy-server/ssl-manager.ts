...
export class SSLManager {
  static getSSLCertificates(domain: string, type: SSLTermination): Promise<SSLCerts> {
  ...
  static async getSelfSignedCertificates(domain: string): Promise<SSLCerts> {
    ...
    if (FileManager.mkdirSync(certPath)) {
      if (!FileManager.isPath(`${certPath}/privkey.key`) && !FileManager.isPath(`${certPath}/cert.crt`)) {
        await CLI.exec(`openssl genpkey -algorithm RSA -out ${certPath}/privkey.key`);
        await CLI.exec(`openssl req -new -key ${certPath}/privkey.key -out ${certPath}/wiredoor.csr -config ${opensslConf}`);
        await CLI.exec(`openssl x509 -req -days 3650 -in ${certPath}/wiredoor.csr -signkey ${certPath}/privkey.key -out ${certPath}/cert.crt`);
      }
  ...
  static async generateDefaultCerts() {
    if (FileManager.mkdirSync(selfSignedCertificatePath)) {
      ...
      if (!FileManager.isPath(defaultCert) && !FileManager.isPath(defaultKey)) {
        ...
        await CLI.exec(`openssl req -new -key ${defaultKey} -out /etc/nginx/ssl/default.csr -config ${opensslConf}`);
        await CLI.exec(`openssl x509 -req -days 3650 -in /etc/nginx/ssl/default.csr -signkey ${defaultKey} -out ${defaultCert}`);
      }
    }
  }
  ...
  static async getCertbotCertificates(domain: string): Promise<SSLCerts> {
    ...
    if (!FileManager.isPath(`${certPath}/privkey.pem`) && !FileManager.isPath(`${certPath}/fullchain.pem`)) {
      const mailOption = config.admin.email ? `-m ${config.admin.email}` : '--register-unsafely-without-email';
  ...
  static getSSLPair(domain: string, type: SSLTermination) {
    let certPath = this.getCertPath(domain, type);
    if (!domain || domain === '_' || type === 'self-signed') {
      return {
        privkey: `${certPath}/privkey.key`,
        fullchain: `${certPath}/cert.crt`
  ...
  static getCertPath(domain: string, type: SSLTermination) {
    if (!domain || domain === '_' || type === 'self-signed') {
      const domainCertFolder = !domain || domain === '_' ? 'default' : domain;
